...
The mixed-model parent orchestrator uses a separate lightweight plan: `{"tasks": [{"id": "...", "model": "flash|pro|luna|terra|sol", "prompt": "...", "contract": "...", "verifiers": [{"argv": ["..."]}]}]}`. Astra assigns exactly one model per task before launch. The orchestrator runs independent tasks concurrently, retains failed worktrees, and never integrates patches automatically.
