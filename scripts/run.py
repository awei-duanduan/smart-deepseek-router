...
def main():
    ...
    p.add_argument("command", choices=["doctor", "plan", "route", "dispatch", "integrate", "orchestrate", "configure"])
    ...
    env = os.environ.copy()
    if args.command in ("doctor", "route", "dispatch", "orchestrate") and not env.get("DEEPSEEK_API_KEY") and os.name == "nt":
        key = load_key()
        if key:
